(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/js/battle_Control.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, '210fe0lFfJEOL9yRavRnXAu', 'battle_Control', __filename);
	...
	battleInit_hero: function battleInit_hero(checkId, personPrefabNew) {
		...
		for (var _i = 0; _i < otherList.length; _i++) {
			...
			for (var _j = 0; _j < this.batlist.length; _j++) {
				//--------战场需要遍历50次，所以用batlist，而不用专门获取的batBox.getChildByName
				var batItem = this.batlist[_j].getComponent("batBox_basic");
				if (batItem.x == otherList[_i].x && batItem.y == otherList[_i].y) {
					batItem.bat_hero = {
						point: otherList[_i].point,
						groupId: 1
						//this.batBox.getChildByName("batBox_y" + batItem.y + "_x" + batItem.x).bat_hero = batItem.bat_hero;
					};this.battleInit_hero_detail(otherList[_i].point, batItem, 1, personPrefabNew);
				}
			}
		}
		for (var _i2 = 0; _i2 < enemyList.length; _i2++) {
			...
			for (var _j2 = 0; _j2 < this.batlist.length; _j2++) {
				//--------战场需要遍历50次，所以用batlist，而不用专门获取的batBox.getChildByName
				var batItem = this.batlist[_j2].getComponent("batBox_basic");
				if (batItem.x == enemyList[_i2].x && batItem.y == enemyList[_i2].y) {
					batItem.bat_hero = {
						point: enemyList[_i2].point,
						groupId: 2
						//this.batBox.getChildByName("batBox_y" + batItem.y + "_x" + batItem.x).bat_hero = batItem.bat_hero;
					};this.battleInit_hero_detail(enemyList[_i2].point, batItem, 2, personPrefabNew);
				}
			}
		}
  ...
  isDie   : 1						//死亡标识   0：死亡，1:未死亡
    ...
    heroRouteOkList.lastId = -1;
			...
			this.other.getChildByName("hero_" + point).on(cc.Node.EventType.TOUCH_CANCEL, function (event) {
				...
				if (boxItem.bat_hero.groupId != 2) {
					//-----------------------------松手时也要检测松手位置的格子上是否有敌人，有就不再加入路线
	...
	update: function update(dt) {
		...
		for (var j = 0; j < heroList.length; j++) {
			...
			if (heroList[j].state == 11) {
				...
				if (heroList[j].groupId == 1) {
					var start = self.other.getChildByName(heroList[j].heroName).position;
				} //-------------获取目标当前xy 
				if (heroList[j].groupId == 2) {
					var start = self.enemy.getChildByName(heroList[j].heroName).position;
				} //-------------获取目标当前xy
				...
				if (heroList[j].groupId == 1) {
					self.other.getChildByName(heroList[j].heroName).setPosition(item);
				} //----------------更新目标位置
				if (heroList[j].groupId == 2) {
					self.enemy.getChildByName(heroList[j].heroName).setPosition(item);
				} //----------------更新目标位置
	...
	//获取英雄目标  ---根据英雄节点名字
	getHeroByName: function getHeroByName(name) {
		//
		for (var i = 0; i < this.hero_list.length; i++) {
			//------------循环英雄数组
			if (this.hero_list[i].heroName == name) {
				//----------匹配当前点击的格子上是否存在英雄
	...
	//鼠标或手指碰触结束   (路线确认数组)
	touchStop: function touchStop(boxItem) {
		var self = this;
		var heroRouteOkList = this.hero_route_ok_list;
		...
		if (heroRouteOkList.list.length != 0) {
			...
			for (var j = 0; j < heroList.length; j++) {
				//---------匹配英雄表
				if (heroList[j].heroName == heroRouteOkList.name) {
					//--------------找出赋值英雄移动数组
...
cc._RF.pop();
        ...
        //# sourceMappingURL=battle_Control.js.map
